defmodule VilanoKernel.Storage.FailureRecovery do
  ...
  defdelegate stop_service_run_instance!(service_run, error_body, reason, now, lease_id), to: ServiceFailure
  defdelegate timeout_result_for_run!(run, error_body, now, lease_id), to: ServiceFailure
  defdelegate fail_service_open_envelope!(service_run, envelope, error_body, reason, now, wake_waiter?), to: ServiceFailure
  ...
  defdelegate fail_service_turn_attempt!(service_run, envelope, error_body, retry_options, now, lease_id), to: RetryRecovery
  ...
  defdelegate cancel_outbound_service_asks!(caller_run_id, error_body, reason, now), to: Cancellation
  defdelegate cancel_service_envelope_by_correlation!(service_run_id, correlation_id, error_body, reason, now), to: Cancellation
  defdelegate cancel_child_runs_for_parent!(parent_run_id, error_body, reason, now), to: Cancellation
end
