...
export function renderDaemonStorage(body: RuntimeStorageResponse): string {
  ...
  return [
    ...
    `  run_payloads: count=${body.database.runPayloads.count} input_output_error=${formatBytes(body.database.runPayloads.bytes)}`,
    ...
    `  service_envelopes: count=${body.database.serviceEnvelopes.count} payload_reply_error=${formatBytes(body.database.serviceEnvelopes.bytes)}`,
    `  run_execs: count=${body.database.runExecs.count} metadata_json=${formatBytes(body.database.runExecs.bytes)}`,
    `  run_steps: count=${body.database.runSteps.count} metadata_json=${formatBytes(body.database.runSteps.bytes)}`,
    `  run_waits: count=${body.database.runWaits.count} output_json=${formatBytes(body.database.runWaits.bytes)}`,
    `  run_signals: count=${body.database.runSignals.count} payload_json=${formatBytes(body.database.runSignals.bytes)}`,
    `  run_service_ops: count=${body.database.runServiceOps.count} payload_response_error=${formatBytes(body.database.runServiceOps.bytes)}`,
    `  topic_publishes: count=${body.database.topicPublishes.count} payload_json=${formatBytes(body.database.topicPublishes.bytes)}`,
  ].join("\n");
}
...
export function renderDaemonPrune(body: RuntimePruneResponse): string {
  return [
    ...
    "project_snapshots:",
    `  grace_seconds: ${body.projectSnapshots.graceSeconds}`,
    ...
    `  failed: ${body.projectSnapshots.failedCount} (${formatBytes(body.projectSnapshots.failedBytes)})`,
    "run_workspaces:",
    ...
    `  failed: ${body.runWorkspaces.failedCount} (${formatBytes(body.runWorkspaces.failedBytes)})`,
    "completed_runs:",
    `  enabled: ${body.completedRuns.enabled}`,
    `  ttl_seconds: ${body.completedRuns.ttlSeconds ?? "disabled"}`,
    `  eligible: ${body.completedRuns.eligibleCount ?? body.completedRuns.candidateCount}`,
    `  candidates: ${body.completedRuns.candidateCount}`,
    `  skipped_unsafe: ${body.completedRuns.skippedUnsafeCount ?? 0}`,
    `  removed: ${body.completedRuns.removedCount}`,
    "service_envelopes:",
    `  enabled: ${body.serviceEnvelopes.enabled}`,
    `  ttl_seconds: ${body.serviceEnvelopes.ttlSeconds ?? "disabled"}`,
    `  candidates: ${body.serviceEnvelopes.candidateCount}`,
    `  removed: ${body.serviceEnvelopes.removedCount}`,
    "artifacts:",
    `  grace_seconds: ${body.artifacts.graceSeconds}`,
    `  candidates: ${body.artifacts.candidateCount} (${formatBytes(body.artifacts.candidateBytes)})`,
    `  removed: ${body.artifacts.removedCount} (${formatBytes(body.artifacts.removedBytes)})`,
    `  failed: ${body.artifacts.failedCount} (${formatBytes(body.artifacts.failedBytes)})`,
    "event_payloads:",
    `  grace_seconds: ${body.eventPayloads.graceSeconds}`,
    `  garbage_collected: ${body.eventPayloads.garbageCollected}`,
    "runtime_cache:",
    `  enabled: ${body.runtimeCache.enabled}`,
    `  ttl_seconds: ${body.runtimeCache.ttlSeconds ?? "disabled"}`,
    `  candidates: ${body.runtimeCache.candidateCount} (${formatBytes(body.runtimeCache.candidateBytes)})`,
    `  removed: ${body.runtimeCache.removedCount} (${formatBytes(body.runtimeCache.removedBytes)})`,
    `  failed: ${body.runtimeCache.failedCount} (${formatBytes(body.runtimeCache.failedBytes)})`,
    "daemon_log:",
    `  enabled: ${body.daemonLog.enabled}`,
    `  max_bytes: ${body.daemonLog.maxBytes ?? "disabled"}`,
    `  previous: ${formatBytes(body.daemonLog.previousBytes)}`,
    `  truncated: ${body.daemonLog.truncated}`,
    `  removed: ${formatBytes(body.daemonLog.removedBytes)}`,
    ...(body.daemonLog.failureReason ? [`  failure: ${body.daemonLog.failureReason}`] : []),
    "database:",
    `  wal_checkpointed: ${body.database.walCheckpointed}`,
    ...(body.database.walCheckpoint
      ? [
          `  wal_checkpoint_busy: ${body.database.walCheckpoint.busy ?? "unknown"}`,
          `  wal_frames: ${body.database.walCheckpoint.logFrames ?? "unknown"}`,
          `  wal_checkpointed_frames: ${body.database.walCheckpoint.checkpointedFrames ?? "unknown"}`,
        ]
      : []),
    `  vacuumed: ${body.database.vacuumed}`,
    `  before: ${formatBytes(body.database.beforeBytes)}`,
    `  after: ${formatBytes(body.database.afterBytes)}`,
  ].join("\n");
}
